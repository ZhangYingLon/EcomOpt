...
class LoginThread(QThread):
    ...
    def __init__(self, platform: BaijiahaoPlatform, username: str, password: str):
        super().__init__()
...
class PublishThread(QThread):
    ...
    def __init__(self, platform: BaijiahaoPlatform, title: str, content: str, **kwargs):
        super().__init__()
        self.platform = platform
        self.title = title
...
class BaijiahaoWidget(QWidget):
    """百家号功能组件"""
    
    def __init__(self):
        super().__init__()
        self.logger = Logger.get_logger("BaijiahaoWidget")
        self.platform: Optional[BaijiahaoPlatform] = None
    ...
    def init_ui(self):
        ...
        self.username_input.setPlaceholderText("请输入用户名/手机号")
        ...
        refresh_btn.clicked.connect(self.refresh_article_list)
    ...
    def handle_login(self):
        ...
        if not self.platform:
            self.platform = BaijiahaoPlatform()
    ...
    def handle_publish(self):
        ...
        self.publish_thread = PublishThread(self.platform, title, content, tags=tags)
    ...
    def ai_generate_title(self):
        ...
        try:
            title = self.ai_service.generate_title(content, platform="baijiahao")
            if title:
    ...
    def ai_optimize_content(self):
        ...
        try:
            optimized = self.ai_service.optimize_content(content, platform="baijiahao")
            if optimized:
    ...
    def refresh_article_list(self):
        ...
        try:
            articles = self.platform.get_article_list()
            self.article_list.clear()
            for article in articles:
                self.article_list.addItem(article.get("title", "未知标题"))
        except Exception as e:
